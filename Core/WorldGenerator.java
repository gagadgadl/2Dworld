...
public class WorldGenerator {
    ...
    public static Position[] createWorld(TETile[][] world, Random random) {
        ...
        connectRoomsInWorld(world, rooms, random); // Connect rooms with hallways with no walls.
        removeRedundantWalls(world); // Cleverly make hallways have walls.
        Position[] PosA=new Position[2];

        PosA[0]=addAvatar(world, rooms, random); // Add an avatar to the world.
        PosA[1]=addGoal(world, rooms, random);
        return PosA;
    }
    ...
    // Add an Goal to the world, within a random room.
    // Return position of the avatar.
    private static Position addGoal(TETile[][] world, Queue<Room> rooms, Random random) {
        List<Room> birthPlaces = new ArrayList<>();
        for (Room room : rooms) {
            birthPlaces.add(room);
        }
        ...
        Room birthRoom = birthPlaces.get(i);
        // Make sure that avatar does not born in wall.
        int GoalX = birthRoom.pos.x + 1 + random.nextInt(birthRoom.width - 2);
        int GoalY = birthRoom.pos.y + 1 + random.nextInt(birthRoom.height - 2);
        world[GoalX][GoalY] = Tileset.FLOWER;
